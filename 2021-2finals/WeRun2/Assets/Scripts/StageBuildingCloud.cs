...
public class StageBuildingCloud : MonoBehaviour
{
    public float StageTipSize = 0;
    ...
    void Start()    //초기화 처리
    {
        //게임 시작 직후의 초기화 처리로서 캐릭터의 위치와는 상관없이 지정된 인덱스 위치에서 지정된 수의 스테이지 팁을 생성
    ...
    void UpdateStage(int toTipIndex)    //스테이지의 업데이트 처리
    {
        ...
        //지정한 스테이지 팁까지를 작성
        for (int i = currentTipIndex + 1; i <= toTipIndex; i++) //1~34까지
        {
